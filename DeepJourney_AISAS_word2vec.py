...
class CoAttn(nn.Module):
    def __init__(self,config):  # p=cnn_out_dim
        ...
        self.co_att_linear_u = nn.Linear(config.DeepShare_bilstm_hidden_size * 2, config.DeepShare_co_attn_dim)
        self.co_att_linear_i = nn.Linear(config.DeepShare_bilstm_hidden_size * 2, config.DeepShare_co_attn_dim)
    ...
    def forward(self,u_vec,i_vec, item_attn_score):
        ...
        user_agg_vec = F.softmax(a_u_i,1) * u_vec #[bs,10,2*50]
        return user_agg_vec
...
class IntentGenerator(nn.Module):
    def __init__(self,config):  # p=cnn_out_dim
        ...
        self.user_hidden_layer = nn.Sequential(
                nn.Linear(config.DeepShare_co_attn_dim * 2 + config.id_embd_num, config.DeepShare_user_hidden_dims[0]),
                # nn.Tanh(),
...
class AttnGreedySearch(nn.Module):
    def __init__(self,config):  # p=cnn_out_dim
        ...
        self.proj = nn.Linear(config.DeepShare_co_attn_dim * 2, config.DeepShare_user_hidden_dims[1])
...
class AttnGreedySearchV3(nn.Module):
    def __init__(self,config):  # p=cnn_out_dim
        super(AttnGreedySearchV3, self).__init__()
        self.search_num = config.DeepShare_search_num
        self.proj = nn.Linear(config.DeepShare_co_attn_dim * 2, config.DeepShare_user_hidden_dims[1])
        self.w_s = nn.Linear(config.DeepShare_user_hidden_dims[1], config.DeepShare_user_hidden_dims[1],bias = False)
        self.w_t = nn.Linear(config.DeepShare_user_hidden_dims[1], config.DeepShare_user_hidden_dims[1],bias = False)
    ...
    def forward(self,user_intent, item_corpus):
        ...
        hidden_size = item_corpus.shape[2]
        item_vec = item_corpus
        attn_score = self.attn(user_intent,item_corpus) #[bs,10]
        score,idx = torch.topk(attn_score,k=self.search_num, dim = 1)
        # self.index.append(idx.detach().numpy().tolist())
        idx = idx.unsqueeze(2).repeat((1,1,hidden_size)) # [bs,3,16]
        item_vec = torch.gather(item_corpus,1,idx) # [bs,3,16]
        user_intent = torch.cat([user_intent,item_vec], dim = 1)
        return user_intent #[bs,1 + search_num, 16]
...
class DeepJourney(nn.Module):
    def __init__(self, config, word_emb,user_num, item_num):
        ...
        self.food_search = AttnGreedySearchV2(config)
        self.service_search = AttnGreedySearchV2(config)
        self.ambience_search = AttnGreedySearchV2(config)
        self.price_search = AttnGreedySearchV2(config)
        ...
        if type(self.Loss_weight)==type(0.0):
            self.Loss_weight=[self.Loss_weight,1e-3]
        ...
        self.fm = FactorizationMachine(config.DeepShare_user_hidden_dims[-1] * 4 * (1 + config.DeepShare_search_num) + config.DeepShare_attn_dim * 2 * 2 + config.id_embd_num * 2, 10)
        self.search_index = []
        self.all_intent = []
    ...
    def forward(self, user_review, item_review, uid,iid, u_iid_seq, i_uid_seq, has_img, user_mean_rating, food_label, service_label, price_label, ambience_label, price_differ, cate_differ,location_differ,hist_post_img): 
        ...
        i_vec_s_word,(i_vec_hn,i_vec_cn) = self.bi_lstm_i_word(i_vec_s) #[bs*10,50,2*150] word_corpus
        i_vec_s = F.max_pool2d(i_vec_s_word,(word_num,1))
        ...
        item_agg_vec,item_attn_score = self.i_attn_w(i_s) #[bs,10,2*50]
        user_agg_vec = self.co_attn_w(u_s,i_s,item_attn_score)
        # item review level aggregation and user item review level co attention
        item_agg_vec_r, item_attn_score_r = self.i_attn_r(item_agg_vec)
        item_agg_vec_r = torch.sum(item_agg_vec_r, dim = 1) #[bs,2*50]
        user_agg_vec_r = torch.sum(self.co_attn_r(user_agg_vec,item_agg_vec,item_attn_score_r), dim = 1) #[bs,2*50]
        ...
        u_out = torch.cat([u_id_embd,user_all_intent,user_agg_vec_r],dim = 1)#[bs,16*4*(search_num+1)+16+2*50]
